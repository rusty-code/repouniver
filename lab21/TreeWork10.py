...
# Необходимо вставить в дерево значение
...
class BinaryTree:
    ...
    def _init_end_bypass(self, node:Node) -> list:
        ...
        if node is None:
            ...
            self._init_end_bypass(node=node.get_left())
            print(node)
        elif node.get_left() is None and not node.get_right() is None:
            self._init_end_bypass(node=node.get_right())
            print(node)
        else:
            self._init_end_bypass(node=node.get_left())
            self._init_end_bypass(node=node.get_right())
